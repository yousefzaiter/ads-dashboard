...
def _build_google_ads_client() -> GoogleAdsClient:
    ...
    The library manages OAuth2 internally: it uses the refresh_token to obtain a
    fresh access_token and automatically refreshes it before it expires.
    """
    config = {
...
def get_credentials() -> str:
    """
    Return a valid access token via the google-ads Python client library.
    The library uses refresh_token + client_id + client_secret to obtain and
    automatically renew the access_token — no static token needed.
    """
    client = _build_google_ads_client()
    creds = client.credentials
    if not creds.valid or creds.expired:
        logger.info("Access token expired — refreshing via google-ads library")
        creds.refresh(_google_requests.Request())
    logger.info("Access token ready")
    return creds.token


def get_headers(access_token: str) -> dict:
    """Build Google Ads REST API request headers."""
    dev_token = os.environ.get("GOOGLE_ADS_DEVELOPER_TOKEN", "")
    if not dev_token:
        raise ValueError("GOOGLE_ADS_DEVELOPER_TOKEN must be set in .env")
...
async def list_accounts() -> str:
    """
    Lists all client accounts directly under the configured MCC (manager account).

    Uses the customer_client resource to query only non-manager accounts linked to
    the login_customer_id (MCC). Accounts outside the MCC are excluded.

    Returns:
        A formatted list of client accounts under the MCC with their IDs and names.
    """
    try:
        if not GOOGLE_ADS_LOGIN_CUSTOMER_ID:
            return "Error: GOOGLE_ADS_LOGIN_CUSTOMER_ID environment variable is not set. This is required to filter accounts under your MCC."

        creds = get_credentials()
        headers = get_headers(creds)

        mcc_id = format_customer_id(GOOGLE_ADS_LOGIN_CUSTOMER_ID)
        url = f"https://googleads.googleapis.com/{API_VERSION}/customers/{mcc_id}/googleAds:search"
        ...
        if not results:
            return "No client accounts found under the configured MCC."

        result_lines = [f"Client Accounts under MCC {mcc_id}:"]
        result_lines.append("-" * 50)

        for row in results:
            client = row.get("customerClient", {})
            customer_id = format_customer_id(str(client.get("id", "")))
            name = client.get("descriptiveName", "(no name)")
            result_lines.append(f"{name} = {customer_id}")
